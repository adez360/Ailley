"""Ailley POC 實驗版 — narrowwindow：串接多輪續寫，改呼叫 continue_director_poc_narrowwindow.py

跟正式版 chain_continue.py 的唯一差異是 subprocess 呼叫的目標腳本，用來驗證
「把前情提要視窗從 12 縮小到 6」對長鏈重複率的影響。

用法：
  python chain_continue_narrowwindow.py <初始 transcript.json> [續寫輪數，預設 5] [每輪回合數，預設 6]
"""
...
def run_chain(initial_path: Path, max_rounds: int, turns_per_round: int) -> dict:
    ...
    for round_i in range(1, max_rounds + 1):
        proc = subprocess.run(
            [sys.executable, str(POC_DIR / "continue_director_poc_narrowwindow.py"), str(current_path), str(turns_per_round)],
