"""Ailley POC 實驗版 — repeat_guard：續寫時偵測重複並給具體正面指令

跟正式版 continue_director_poc.py 的差異只有一個：讀取 prior_turns 之後，
先比對最近幾回合同一位發言者的 reasoning/dialogue 有沒有近乎重複的句子；
如果有，把「具體偵測到的那句話」引用回 prompt，指示模型針對同樣情境換一個
新角度／新細節。這是正面指令（「請想一個不同的...」），不是否定式的
「禁止/不可以」，因為早期實驗證實負面式強語氣規則會讓這顆模型把「整段清空」
當成最安全的合規解（見 note：MAX_TURNS 縮短驗證那段）。

實驗結果驗證通過才會併入正式版 continue_director_poc.py，見計畫文件。

用法：
  python continue_director_poc_repeat_guard.py <transcript.json> [額外回合數，預設 6]
...
def detect_recent_repetition(prior_turns: list, window: int = REPEAT_GUARD_WINDOW,
    ...
    回傳每位發言者最多一筆最新命中的紀錄（避免重複指令塞爆 prompt）。"""
    recent = prior_turns[-window:]
...
def build_repeat_guard_block(findings: list) -> str:
    ...
    for f in findings:
        ...
        lines.append(
            f"- {village}最近的{field_label}已經出現過跟這句很像的內容：「{f['text']}」。"
            f"這次輪到{village}發言時，請針對同樣的情境想一個具體不同的新角度、新細節或新的問法來表達，"
            f"用字遣詞也要跟這句不一樣，讓劇情感覺持續往前推進。"
        )
    return "\n".join(lines)
...
def main() -> None:
    ...
    repeat_findings = detect_recent_repetition(prior_turns)
    repeat_guard_block = build_repeat_guard_block(repeat_findings)
    if repeat_findings:
        ...
        print("[repeat_guard] 未偵測到重複，不加額外指令")
