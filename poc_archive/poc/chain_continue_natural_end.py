"""Ailley POC — 方向 C 實驗：串接測試「自然收尾」機制能不能讓長對話自然結束

跟 chain_continue.py 邏輯一樣（重複呼叫續寫腳本、直到洩漏或跑滿輪數），差異是改呼叫
continue_director_poc_natural_end.py，多偵測一種停止條件：對話自然收尾（懷疑度低迷、
模型判斷該找藉口結束）。

用法：
  python chain_continue_natural_end.py <初始 transcript.json> [續寫輪數，預設 4] [每輪回合數，預設 6]
"""
...
def run_chain(initial_path: Path, max_rounds: int, turns_per_round: int) -> dict:
    ...
    for round_i in range(1, max_rounds + 1):
        proc = subprocess.run(
            [sys.executable, str(POC_DIR / "continue_director_poc_natural_end.py"), str(current_path), str(turns_per_round)],
        ...
        if leaked:
            ...
            leak_line = m_leak.group(0) if m_leak else None
        natural_end_line = None
        if natural_ended:
            m_end = re.search(r"=== 第 \d+ 回合：對話自然收尾[^\n]*===", stdout)
            natural_end_line = m_end.group(0) if m_end else None
            # 找出收尾那句台詞方便人工複核
            m_dialogue = re.search(r"「([^」]*)」\s*\n\s*\n=== 第 \d+ 回合：對話自然收尾", stdout)
            ending_dialogue = m_dialogue.group(1) if m_dialogue else None
        else:
            ending_dialogue = None
        ...
        log["rounds"].append({
            ...
            "ending_dialogue": ending_dialogue,
        })
...
def main():
    ...
    print(f"[串接開始] 起點：{initial_path.name}，最多續寫 {max_rounds} 輪，每輪 {turns_per_round} 回合（目標上限 {6 + max_rounds * turns_per_round} 回合）")
    ...
    out_path = POC_DIR / "transcripts" / f"chain_natural_end_log_{initial_path.stem}.json"
