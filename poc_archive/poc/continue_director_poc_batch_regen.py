"""Ailley POC 實驗版 — batch_regen：生成後偵測批次內重複，偵測到就整批重新生成

跟 repeat_guard 版（continue_director_poc_repeat_guard.py）的差異：repeat_guard 只在「呼叫模型
之前」比對已經寫進歷史的舊回合，對「這次呼叫正在生成的 6 回合」內部彼此重複完全沒有插手機會
（模型是一次性把 6 回合生完的）。這次驗證證實這正是重複退化的主要來源，repeat_guard 對此無效。

這一版改成「生成後檢查」：呼叫完之後，額外比對這次新生成的 new_turns 彼此之間、以及跟最近歷史
有沒有近乎重複；有的話直接整批重新生成一次（帶著這次抓到的具體重複句當新的 repeat_guard 提示），
最多重試 AILLEY_MAX_REGEN_ATTEMPTS 次。仍然沿用正面表述（引用具體句子＋要求換新角度），不用
「禁止/不可以」這種否定式措辭（見 note：早期實驗證實負面式強語氣規則會讓模型把整段清空）。

v2 修訂（第一版驗證後追加）：發現兩個限制效果的問題，這版針對性修正：
1. 「卡住的鏈」單純重打同一批換不出真正新內容——新增 STUCK_TOPIC_HOOKS／
   build_stuck_topic_instruction()，偵測到卡住時強制指定一個具體新話題切入點，
   不再只靠模型自己想新角度。判斷邏輯跟指令措辭沿用自然收尾機制 v3→v4 驗證出的
   教訓：軟性許可對這個模型不夠有拉力，卡住時要用「強制規定」。
2. 模型會把 repeat_guard 提示的措辭本身複誦進 reasoning/dialogue——build_repeat_guard_block()
   改成只引用重複句前段、明確標註是劇本安排提示，並加一句正面表述要求輸出維持角色真實發言。

實驗結果驗證通過才會考慮併入正式版 continue_director_poc.py，見計畫文件。

用法：
  python continue_director_poc_batch_regen.py <transcript.json> [額外回合數，預設 6]
...
# 一批生成內部（或跟最近歷史相比）偵測到重複時，最多重打幾次。
MAX_REGEN_ATTEMPTS = int(os.environ.get("AILLEY_MAX_REGEN_ATTEMPTS", "3"))
...
# ---------------------------------------------------------------------------
# 重複偵測共用邏輯
# 只看「同一位發言者」自己前後的 reasoning/dialogue 有沒有近乎重複——因為第
# 一次驗證發現重複幾乎都是同一角色隔一輪講給自己聽/講給對方聽的同一套話，
# 不是紅藍互相抄對方。抓到就把具體那句話引用回 prompt，指示換一個新角度，
# 不用「禁止/不可以」這種否定式措辭（早期實驗證實會讓模型把整段清空）。
# ---------------------------------------------------------------------------

REPEAT_GUARD_WINDOW = 12  # 跟 PRIOR_TURNS_BLOCK 看的視窗一致，只比對玩家在前情提要裡看得到的範圍
REPEAT_GUARD_THRESHOLD = 0.8


def detect_recent_repetition(prior_turns: list, window: int = REPEAT_GUARD_WINDOW,
                              threshold: float = REPEAT_GUARD_THRESHOLD) -> list:
    """呼叫模型「之前」用：比對最近 window 回合內，同一位發言者的 reasoning/dialogue
    是否有相似句子。回傳每位發言者最多一筆最新命中的紀錄（避免重複指令塞爆 prompt）。"""
    recent = prior_turns[-window:]
        ...
        for i in range(len(items)):
            for j in range(i + 1, len(items)):
                ...
                for field in ("dialogue", "reasoning"):
                    ...
                    if ratio >= threshold:
                        # 同一位發言者只留下最新（j 較大）的一筆命中紀錄
                        findings[speaker] = {"speaker": speaker, "field": field, "text": b, "ratio": ratio}
    return list(findings.values())


def detect_batch_repetition(prior_turns: list, new_turns: list, context_window: int = REPEAT_GUARD_WINDOW,
                             threshold: float = REPEAT_GUARD_THRESHOLD) -> list:
    """呼叫模型「之後」用：檢查這次剛生成的 new_turns 有沒有制造新的重複——
    包括跟最近歷史重複，或 new_turns 內部彼此重複。只在「較晚出現的那句話屬於
    new_turns」時才記錄，已經寫進歷史、改不了的舊重複不重複記。"""
    context = prior_turns[-context_window:]
    combined = context + new_turns
    new_start = len(context)

    by_speaker: dict[str, list[tuple[int, dict]]] = {}
    for idx, turn in enumerate(combined):
        speaker = turn.get("speaker", "?")
        by_speaker.setdefault(speaker, []).append((idx, turn))
    ...
    for speaker, items in by_speaker.items():
        for i in range(len(items)):
            for j in range(i + 1, len(items)):
                idx_i, turn_i = items[i]
                idx_j, turn_j = items[j]
                if idx_j < new_start:
                    continue  # 兩句都在既有歷史裡，這次生成改不了，不算這次的問題
                for field in ("dialogue", "reasoning"):
                    a, b = turn_i.get(field, ""), turn_j.get(field, "")
                    if not a or not b:
                        continue
                    ...
                    if ratio >= threshold:
                        findings.append({
                            "speaker": speaker, "field": field, "text": b, "ratio": ratio,
                            "new_turn_index": idx_j - new_start,
                        })
    return findings


def build_repeat_guard_block(findings: list) -> str:
    # v2 修訂：v1 版本整句引用偵測到的重複句，結果模型偶爾會把提示句式本身
    # （「已經出現過跟這句很像的內容」）直接複誦進 reasoning/dialogue，
    # 讓防重複指令自己變成新的重複素材。這版只引用重複句的前段（降低可複製
    # 的素材量），並明確標註這是劇本安排提示、不是村民心聲，加一句正面表述
    # 要求輸出維持角色真實的想法與發言、不需要提到「已經講過」這件事。
    if not findings:
        return ""
    lines = ["【提醒：避免內容打轉】"]
    for f in findings:
        village = "紅村" if f["speaker"] == "red" else "藍村"
        field_label = "台詞" if f["field"] == "dialogue" else "內心想法"
        excerpt = f["text"][:24]
        lines.append(
            f"- （劇本安排提示，不是{village}的心聲）{village}最近的{field_label}已經很接近這句話的意思："
            f"「{excerpt}...」。這次請讓{village}用完全不同的具體切入點來表達同樣的情境——"
            f"新的細節、新的問法、新的態度都可以，但輸出的 reasoning／dialogue 本身要維持是這位村民"
            f"當下真實的想法與發言，不需要提到「已經講過」這件事。"
        )
    return "\n".join(lines)


# ---------------------------------------------------------------------------
# 卡住偵測：連續重複代表這幾輪已經沒有真正的新素材可聊，單純重打換運氣沒用，
# 改成強制指定一個具體的新話題切入點——呼應自然收尾機制第三版（軟性許可）
# 失敗、第四版（強制指令）才成功的教訓，同一套哲學套用在「換話題」上。
# ---------------------------------------------------------------------------
...
def build_stuck_topic_instruction(hook: str, speaker: str) -> str:
    village = "紅村" if speaker == "red" else "藍村"
    return (
        f"【強制規定：這批新增回合裡必須帶入一個新話題切入點，這不是可以自行選擇的事】\n"
        f"外部系統判斷：最近的內容已經在原地打轉，光靠換句話說沒有用，需要真正新的素材才能讓對話"
        f"繼續往前推進。具體規定：這批新增回合裡，要有一次由{village}的角色自然帶出以下這件近況、"
        f"當作新話題的切入點（可以搭配角色個性自由發揮怎麼講、用什麼態度講，但這件事本身一定要出現，"
        f"這不是機密，只是尋常的日常近況）：\n"
        f"「{hook}」"
    )
...
def main() -> None:
    if len(sys.argv) < 2:
        print("用法：python continue_director_poc_batch_regen.py <transcript.json> [額外回合數，預設 6]")
        sys.exit(1)
    ...
    pre_call_findings = detect_recent_repetition(prior_turns)
    if pre_call_findings:
        for f in pre_call_findings:
            village = "紅村" if f["speaker"] == "red" else "藍村"
            print(f"[repeat_guard] 偵測到{village}近期重複（{f['field']}，相似度{f['ratio']:.2f}）："
                  f"「{f['text'][:40]}」，已把具體指令塞進 prompt")
    else:
        print("[repeat_guard] 未偵測到既有歷史重複")

    world_lore = (POC_DIR / "prompts" / "world_lore.txt").read_text(encoding="utf-8")
    prompt_template = (POC_DIR / "prompts" / "experiments" / "continue_system_prompt.experiment_repeat_guard.txt").read_text(encoding="utf-8")
    ...
    # ------------------------------------------------------------------
    # batch_regen 核心：呼叫後偵測這批新回合有沒有制造新的重複，有的話
    # 帶著具體證據整批重新生成，最多試 MAX_REGEN_ATTEMPTS 次。
    # ------------------------------------------------------------------
    regen_findings_log: list[list[dict]] = []
    ...
    while True:
        attempt += 1
        combined_findings = pre_call_findings + extra_findings
        # 卡住判斷：這一輪打從一開始歷史就已經在重複（pre_call_findings），
        # 或這一輪已經失敗過一次（attempt >= 2）——單純重打同一批換不出新東西，
        # 強制指定一個具體新話題切入點。
        stuck = bool(pre_call_findings) or attempt >= 2
        repeat_guard_block = build_repeat_guard_block(combined_findings)
        if stuck:
            speakers_involved = {f["speaker"] for f in combined_findings} or {"red", "blue"}
            target_speaker = random.choice(sorted(speakers_involved))
            hook = random.choice(STUCK_TOPIC_HOOKS)
            stuck_instruction = build_stuck_topic_instruction(hook, target_speaker)
            repeat_guard_block = (repeat_guard_block + "\n\n" + stuck_instruction).strip()
            village = "紅村" if target_speaker == "red" else "藍村"
            print(f"[batch_regen] 第 {attempt} 次嘗試判定為卡住狀態，加入強制新話題指令"
                  f"（{village}）：「{hook}」")
        prompt = (
            prompt_template.replace("{{WORLD_LORE}}", world_lore)
            .replace("{{ENDING_INSTRUCTION}}", ending_instruction)
            .replace("{{REPEAT_GUARD_BLOCK}}", repeat_guard_block)
        ...
        # 固定 seed 的控制實驗模式下，重試要跟著換 seed，否則會一直生出一樣的東西。
        ...
        candidate_turns = result.get("turns", [])
        ...
        batch_findings = detect_batch_repetition(prior_turns, candidate_turns)
        regen_findings_log.append(batch_findings)

        if not batch_findings:
            new_turns = candidate_turns
            print(f"[batch_regen] 第 {attempt} 次嘗試耗時 {elapsed:.2f} 秒，未偵測到批次內重複，採用這次結果。")
            break

        for f in batch_findings:
            village = "紅村" if f["speaker"] == "red" else "藍村"
            print(f"[batch_regen] 第 {attempt} 次嘗試（耗時 {elapsed:.2f} 秒）偵測到{village}批次內重複"
                  f"（{f['field']}，相似度{f['ratio']:.2f}）：「{f['text'][:40]}」")

        if attempt >= MAX_REGEN_ATTEMPTS:
            new_turns = candidate_turns
            print(f"[batch_regen] 已達最大重試次數（{MAX_REGEN_ATTEMPTS}），仍有重複，採用最後一次結果。")
            break

        print(f"[batch_regen] 重新生成（第 {attempt + 1} 次嘗試）...")
        extra_findings = batch_findings

    print(f"[完成] 續寫共嘗試 {attempt} 次、總耗時 {elapsed_total:.2f} 秒，最終採用 {len(new_turns)} 個新回合。")
    ...
    out_record = {
        ...
        "regen_findings_log": regen_findings_log,
