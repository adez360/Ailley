"""Ailley POC 實驗版 — repeat_guard：串接多輪續寫，改呼叫 continue_director_poc_repeat_guard.py

跟正式版 chain_continue.py 的唯一差異是 subprocess 呼叫的目標腳本，
用來驗證 repeat_guard 的重複偵測+具體正面指令是否能降低長鏈重複率。


重複呼叫 continue_director_poc.py，每次都接在上一次輸出的最新 transcript 後面，
直到（a）洩漏發生、遊戲照規則結束，或（b）跑滿指定的續寫輪數，藉此觀察：
- 洩漏機率隨回合數拉長是否有累積效應，還是持平在 0（如果持平，代表現有戰術規則
  沒有「壓力累積到一定程度必須犯錯」的收斂機制，單純疊續寫次數不會自然導向洩漏）。
- 拉長到 30+ 回合後，敘事會不會變得空洞重複（即使每個 6 回合 chunk 內部是乾淨的）。

用法：
  python chain_continue.py <初始 transcript.json> [續寫輪數，預設 5] [每輪回合數，預設 6]
"""
...
def run_chain(initial_path: Path, max_rounds: int, turns_per_round: int) -> dict:
    ...
    for round_i in range(1, max_rounds + 1):
        proc = subprocess.run(
            [sys.executable, str(POC_DIR / "continue_director_poc_repeat_guard.py"), str(current_path), str(turns_per_round)],
