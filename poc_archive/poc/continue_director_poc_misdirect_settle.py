"""Ailley POC 實驗版 — misdirect_settle：misdirect 只標在「轉移那一句」，換完話題就回到 none

跟正式版 continue_director_poc.py 的唯一差異：讀取的 prompt 換成
prompts/experiments/continue_system_prompt.experiment_misdirect_settle.txt，
戰術決策規則多一句「misdirect 只用在主動轉開話題的那一句，轉開之後沒有重新被逼近
就回到 none」。動機：觀察到長鏈裡 misdirect 常常連續掛十幾回合，但 suspicion_change
大多是 0 或負值、內容也確實跟機密無關，代表 misdirect 標籤已經跟遊戲狀態脫鉤，
變成模型的預設低風險標籤，不是真的在承受追問壓力下的迴避動作。

刻意跟 batch_regen（生成後偵測重複、重打）分開測，這是不同的變因，避免混淆是哪個
因素造成效果。其餘邏輯（自然收尾、跨陣營洩題偵測、OpenCC）完全比照正式版不動。

實驗結果驗證通過才會考慮併入正式版 director_system_prompt.txt／continue_system_prompt.txt，
見計畫文件。

用法：
  python continue_director_poc_misdirect_settle.py <transcript.json> [額外回合數，預設 6]
...
def main() -> None:
    ...
    prompt_template = (POC_DIR / "prompts" / "experiments" / "continue_system_prompt.experiment_misdirect_settle.txt").read_text(encoding="utf-8")
