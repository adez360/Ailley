"""Ailley POC 實驗版 — memory_embed：記憶流補上相關性檢索（embedding）

跟 dialogue_ping_pong_memory.py 的差異：那版只做「新近度＋重要性」，相關性（embedding
向量檢索）當時缺基礎設施沒做。這版補上：另外部署一顆小型 embedding 模型（bge-small-zh-v1.5，
獨立 llama-server 服務跑在 8081，不影響現有對話 server 的 8080），每筆記憶寫入時多存一份
embedding 向量，檢索時額外算一項「跟最近一句話的語意相關性」，跟新近度、重要性加總排序。

這不是要解決重複退化本身（那個已經確認主力是「限制長度」，見 note）——這次的價值主要是
驗證「長期記憶檢索品質」這個 6 個月尺度世界會用到的能力補齊了沒有，重複率有沒有額外幫助
是一併觀察的副產物，不是主要訴求。

用法：
  python dialogue_ping_pong_memory_embed.py
...
def get_embedding(text: str) -> list[float]:
    """呼叫 embedding server（bge-small-zh-v1.5，跟對話 server 是不同的獨立服務、不同 port）。
    回應格式是 [{"embedding": [[...]]}]，實際向量在多包一層的內層 list 裡，這裡拆開。"""
    try:
        ...
        print(f"[embedding] 呼叫失敗（{e}），這筆記憶的相關性會退化成 0")
        return []
...
def retrieve_memories(memories: list, current_turn: int, top_k: int = MEMORY_TOP_K,
                       query_embedding: list[float] | None = None) -> list:
    """新近度（指數衰減）+ 重要性（正規化到 0-1）+ 相關性（跟最近一句話的 cosine similarity）
    加權排序，取分數最高的 top_k 筆，依原始回合順序排列後回傳，方便閱讀時保持時間軸連貫。
    query_embedding 沒有值時（例如還沒有任何歷史），相關性一律當 0，優雅降級成原本的
    新近度＋重要性版本。"""
    scored = []
    for m in memories:
        recency = MEMORY_RECENCY_DECAY ** (current_turn - m["turn"])
        importance = m["importance"] / 10
        relevance = cosine_similarity(m.get("embedding", []), query_embedding) if query_embedding else 0.0
        scored.append((recency + importance + relevance, m))
...
def call_director(prompt: str, grammar: str, seed_override: int | None = None, n_predict: int = -1) -> dict:
    payload = {
        "prompt": prompt,
        "grammar": grammar,
...
def main() -> None:
    ...
    for i in range(1, MAX_TURNS + 1):
        ...
        # 查詢基準：最近一句話（shared_log 最後一筆），還沒有歷史時相關性項優雅降級成 0。
        query_embedding = get_embedding(shared_log[-1]["dialogue"]) if shared_log else None
        retrieved = retrieve_memories(memory_stream[speaker], current_turn=i, query_embedding=query_embedding)
        ...
        memory_embedding = get_embedding(f"{reasoning} {dialogue}")
