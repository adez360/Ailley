...
| 效能 | `/decide` 逾時建議值 | 各 provider 可設定 `timeout`（`AIConfig._parse_provider()` 讀取、`AIService` 逐次請求套用），缺值或非正值退回 `ai_config.gd::DEFAULT_TIMEOUT`（10 秒） | 《04》§6、《99》P-11 |
...
| 對話機制（新機制，待補完整規格） | 待規劃 | `talk` 現況已是多輪 `Conversation` 物件（見 `技術/talk 動作設計.md`），MVP 沿用，**不是一次性單句**；`murmur` 才是一次性單句，見上一列。等待對方回話的逾時秒數對接《12》§6.2（P-22 待填）；**現況（2026-08-24 更新，見《99》P-23）**：三人以上同場對話已拉進這學期 MVP 範圍，但排在最後——技術範圍已定義（`conversation.gd` 核心資料模型要換，牽動至少 5 個檔案，誰先講/誰決定結束的仲裁邏輯待設計），只是不排進這學期發表前的實作排程，動工前程式碼還是現有 2 人限制 | MVP-1 |
| UI 補齊 | 已完成 | 角色狀態面板**已實作**（`status_panel.gd`）。**現況（2026-08-23 查證）**：原本列為「MVP-1 需要、目前完全沒做」的三個面板都已存在——`god_stone_input.gd`／`.tscn`（天神之石輸入框）、`status_panel.gd` 三分頁含 `today_plan`／`today_log`（今日摘要，見《技術/角色資訊面板》）、`inventory_panel.gd`／`hotbar.gd`（身上物品）。快捷欄／物品欄 UI 雖然是給 player 用的，MVP-2 玩家接上後即可直接使用。**詳細規格見《15》**；建角面板的「決策來源」欄位與造型組挑選 UI 見《05》§3-1／§5-0 | MVP-1 |
