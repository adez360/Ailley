...
graph TB
    ...
    Agent -->|dialogue 信封| AIS
```

**現況與目標的落差**（勿誤讀成已完成）：`AIService`（正式線）已接上對話（`Agent.next_line()`），但**行程決策迴圈尚未接上**——仲裁器只吃 schedule 來源的任務；`DecisionProvider` 介面**尚未實作**，現況是直接呼叫 llama-server。完整落差清單見 `ai/api.md`「已知缺口」與 `技術/腳本地圖.md`「目前還沒做的」。
...
| 《01》/《01-1》角色數值＋建角 | 簡化 | 六維滑桿、10 項人格、`system_prompt`、`words_to_creator` 全做；外觀**不拆 slot**，改提供幾套固定「造型組」整套挑選（取代 P-01 的完整外觀系統，那個留到之後） | MVP-1 |
...
| 《07》搬運（新機制，待補完整規格） | 待規劃 | 昏迷者可被他人搬運：搬運中在畫面上可見（搬運者拖／抱著對方移動）、消耗搬運者體力、搬運者對被搬運者可做其他互動；**不做全村廣播**，要走到附近才會知道有人昏迷／被搬。細節（搬運速度、體力消耗量、可中斷條件）尚未定案，新增追蹤見《99》 | MVP-1 |
| 《08》物品與經濟 | 簡化 | 餐酒館沿用**已實作的販賣機機制**（`技術/販賣機.md`）當簡化版，不重新設計交易流程；藥草鋪同樣沿用；正式餐酒館玩法留到 MVP-2 | MVP-1 |
| 《09》死亡與復活 | 大幅簡化，改「昏迷」 | MVP 不做死亡／安葬／墓園全套，改用「昏迷」：跟死亡一樣石化原地，但可被他人搬運帶回家（不是搬去墓園），沒有復活流程（本來就沒死）。完整死亡系統留到之後。細節（昏迷觸發門檻、能否自行甦醒）尚未定案，新增追蹤見《99》 | MVP-1 |
| 《10》世界模式與連線 | 大幅簡化 | 世界一律單機跑；「允不允許 player 加入」這個建立時旗標 MVP 先不開放設定（預設允許），正式的模式選擇留到之後；連線／多人／導演權轉移／席位付費全部不做，但《12》的 `DecisionProvider` 介面設計本身要保留可擴充性，之後接 `RemotePlayer` 不用重構 | MVP-1 |
| 《11》人際互動 | 簡化 | `talk`／`give`／`persuade`／`shout` 全做；`perform` 留到工作系統完整化後；`report` 跟偷竊、《洗心革面所》綁在一起延後；§3 群體對話／八卦／聲望細緻後果不進 MVP（已追蹤於《99》P-23） | MVP-1（`perform`/`report` 除外） |
| 對話機制（新機制，待補完整規格） | 待規劃 | `talk` 現況已是多輪 `Conversation` 物件（見 `技術/talk 動作設計.md`），MVP 沿用，**不是一次性單句**；等待對方回話的逾時秒數對接《12》§6.2（P-22 待填）；三人以上同場對話 MVP 先維持現有 2 人限制，第三人在場時的處理方式待規劃（已追蹤於《99》P-23） | MVP-1 |
| UI 補齊 | 部分已有 | 角色狀態面板**已實作**（`Ailley/scripts/ui/status_panel.gd`，含姓名/年齡/金錢/全部 Stats）；今日摘要面板（`today_plan`／做過什麼）與身上物品面板待做；缺的 UI 先用文字佔位（沿用專案既有做法：`status_panel.gd` 的 `AGE_PLACEHOLDER := "—"`，佔位置不留空白也不顯示假數值） | MVP-1 |
