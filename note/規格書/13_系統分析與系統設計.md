...
| 《09》死亡與復活 | 大幅簡化，改「昏迷」 | MVP 不做死亡／安葬／墓園全套，改用**新狀態**「昏迷」（`health` 歸零觸發，跟既有「暈倒」`stamina` 歸零不是同一件事）：跟死亡一樣石化原地，可被搬運（目的地見上一列），沒有復活流程（本來就沒死）。完整死亡系統留到之後。細節見《99》P-27 | MVP-1 |
...
| UI 補齊 | 部分已有 | 角色狀態面板**已實作**（`status_panel.gd`）。**快捷欄／物品欄 UI 已實作但是給 player 用的**——MVP-1 還沒接 player，暫時用不到，等 MVP-2 玩家加入才需要；MVP-1 真正需要、目前完全沒做的是**天神之石輸入框**。今日摘要面板（`today_plan`／做過什麼）與身上物品面板也待做。缺的 UI 都要有對應機制/資料同步做出來，不是純版面；做之前先用文字佔位（沿用 `status_panel.gd` 的 `AGE_PLACEHOLDER := "—"` 做法，不留空白也不顯示假數值） | MVP-1 |
