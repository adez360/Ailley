...
| 《07》地點與行動 | 簡化 | 6 個地點：家／藥草鋪／餐酒館／涼亭／天神之石／藥草叢（2026-08-24 拍板拉入，issue #574）；工作簡化成**單一工作站，站著即可賺錢**（沿用已實作的 `技術/工作站.md` 機制，不用另外設計）；藥草叢僅開放 `gather`，單次判定產出 `herb`，不做資源存量／再生；戰鬥新增 `attack`（毆打）；新增「搬運」子機制見下一列 | MVP-1 |
