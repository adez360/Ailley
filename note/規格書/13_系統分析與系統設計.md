...
| 效能 | 決策逾時預算 | 各 provider 可設定 `timeout`（`AIConfig._parse_provider()` 讀取、`AIService` 逐次請求套用），缺值或非正值退回 `DEFAULT_TIMEOUT` 10 秒（`Ailley/scripts/ai/ai_config.gd`，見《99》P-11） | 《12》§9、《04》§6 |
