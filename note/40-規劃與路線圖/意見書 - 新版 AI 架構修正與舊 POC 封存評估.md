...
> [!info] 決策脈絡
> 引擎確定改用 **Godot**（取代 [[技術架構總覽]] 原訂的 Phaser 路線）。同時新的村民 AI 規格 [[Specify1]]、[[Specify2]]、[[actor1]]（2026-08-10 已從 `neon/` 搬進 `note/90-封存與參考/`）定義了一個 tick-based 開放世界村莊生活模擬，跟現行 `poc_agent_loop` 的「紅村／藍村獻祭博弈」場景是不同的遊戲內容。本文件只做評估與提案，**未執行任何檔案異動**，待核准後才動手。
