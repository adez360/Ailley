...
@export var village_ai_enabled := false      # demo 開關，繞過任務池，見下方 poc_village_sim 段落
@export var poc_character_id := ""           # village_ai_enabled 開啟時對應到 poc 的哪個內部 id
@export var llm_decision_enabled := false    # 正式決策迴圈開關（issue #88），逐隻手動開

const NOTICE_PAUSE := 2.0
const SCHEDULE_BASE_PRIORITY := 10.0 · TIME_BONUS := 100.0
const HYSTERESIS := 5.0 · MIN_COMMIT := 2.0
...
var _tasks: Array[Dictionary]                # 候選池，schedule 來源開場建立一次，llm 來源用 _push_llm_tasks() 加
var _current_task: Dictionary
var current_place: String · current_state: String

func is_interruptible() -> bool              # 覆寫 super() and _current_task.get("interruptible", true)
func exit_conversation() -> void             # 覆寫：講完重算 _reevaluate()
func get_task_debug_info() -> Array[Dictionary]        # debug_console.gd 的 tasks 指令用
...
_on_move_finished()：move_finished 是共用訊號（goto 指令／R&D 線也會發），
...
spotted 且 village_ai_enabled 且對方是玩家 → 觸發一次 VillageSimDecision（見下方）
  ⚠ 這條路完全繞過任務池，不進 _tasks，跟 llm_decision_enabled 是兩個獨立機制
...
AIService（正式線）已接對話（conversation.gd 非同步）與行程（agent.gd 任務池+決策迴圈，
  llm_decision_enabled 開關）；決策內容有效性未實跑真實 provider 驗證過（見驗收清單）
poc_village_sim（R&D 驗證線）已有呼叫端（見上方 Agent／village_sim_*.gd），
  但只證明管線通、決策內容有效性未驗證，且不是出貨架構；跟正式線的 llm_decision_enabled
  是兩條不同的路，沒有防呆擋住同一隻 Agent 同時開兩個
```
