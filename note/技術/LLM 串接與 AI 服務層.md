...
**現在的預設值是 150，不是 30**——30 太容易在單一角色一天內講超過幾場對話
就撞到上限、對話提早收尾，改成 150（issue #655 查證期間一併調整，不是這次
新的取樣研究結果）。下面「預設值 30 的依據」記錄的是 #395 當初怎麼推出
起始值 30 的方法論，數字本身已經不是目前生效的設定，但推算過程（單輪成本、
per-角色配額怎麼算）留著給之後想重新校準時參考。
