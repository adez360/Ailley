...
> [!note] 已有一條繞過本篇整個分層的 AI 說話路徑，但不是這裡規劃的東西
> `village_ai_enabled` 開啟時，`VillageSimDecision.apply()` 決定要說話會
> 直接呼叫 `character.say()`，完全不經過 `Conversation` 節點、沒有輪次、
> 沒有對方同意。這是 R&D 驗證線（見 [[LLM 串接與 AI 服務層]]），
> 不是本篇規劃要接的正式對話路徑，兩者現在並存。
