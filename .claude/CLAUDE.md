# CLAUDE.md

## 筆記庫規則 (Obsidian Vault Rules)

本專案使用 `note/` 目錄作為 Obsidian 筆記庫 (vault)。

- **專案的所有文件都在 `note/`**，一則都不例外：想法、計畫、進度、技術決策，
  以及 API 參考、指令速查這類純查閱資料。
  唯一的例外是 repo 根目錄的 `README.md`（GitHub 入口，給還沒進 vault 的人看）。
- **不要在別的地方另開文件目錄。** 尤其不要在 Godot 專案裡開 `Ailley/docs/` ——
  文件散成兩處之後，vault 就失去「一個地方查得到全部」的意義，
  而且寫在 vault 外的 `[[wikilink]]` 全部是死的。
- 操作筆記庫時，**使用 Obsidian CLI**（`obsidian <subcommand>`，需 Obsidian app 執行中）
  來讀取、建立、搬移、搜尋與管理筆記。
- 撰寫筆記時使用 Obsidian Flavored Markdown：wikilinks（`[[...]]`）、embeds、callouts、frontmatter properties、tags。
- 在開始或推進任何工作前，先到 `note/` 查閱相關筆記；完成後把進度與決策更新回筆記庫。

### vault 結構

依**誰讀**分三層。分類靠資料夾 + frontmatter，不要再引入編號資料夾
（曾經的 `10-` / `30-` 只是把第一個 tag 再寫一次）。

| 資料夾 | 讀者 | 裝什麼 |
| --- | --- | --- |
| `交流/` | 人類 ⇄ AI | **暫存**的溝通區：現況、待拍板的問題。**不寫技術細節**，溝通完就刪掉 |
| `技術/` | 人類與 AI | API 技術文件：各系統怎麼運作、有哪些介面可叫、為什麼這樣設計、踩過的坑 |
| `ai/` | **AI** | 密集格式的參考資料。不必是散文，人類不必讀 |

`note/規格書/` 是另一塊，跟上面三層平行：完整的產品規格集，自成 00~99 的編號體系，
依編號閱讀、00 是上位文件。不算三層之一，不套用上面「誰讀」的分法。

規格書與 `技術/`／`ai/` 筆記內容不一致時，**以規格書為準**。技術/ai 筆記裡有、
規格書沒有寫的具體規格，要補進規格書對應章節；技術/ai 筆記跟規格書兜不起來的，
列入《99_待規劃項目清單》待決策，不要各自為準各寫各的。

《99》的每一列只記內容（問題是什麼、選項有哪些），不記工作流。
「誰負責、何時要有結果」改開 GitHub issue 追蹤，issue 編號填回《99》
對應列的「負責」／「期限」欄；不要在表格裡手填人名與日期——
沒有東西會提醒你更新它，issue 才有指派通知與到期提醒。

入口是 `note/Ailley.md`，新增 `技術/` 或 `ai/` 筆記後回去對應表格加一列。
`交流/` 的筆記不進表格 —— 它們會被刪掉。

### `交流/` 是暫存的，不要從外面引用

`交流/` 裝的是「這件事還在談」。談完、拍板完，那則筆記就被移除。所以：

- **其他地方不要引用 `交流/`。** `技術/`、`ai/`、`Ailley.md`、issue、PR、commit
  都不要用 `[[wikilink]]` 或路徑指進去 —— 被引用的筆記隨時會消失，連結全部變死的。
- 要長期留著的結論，**抄進 `技術/` 或 `ai/`**，寫成該系統自己的一段，
  而不是留一條「詳見 `交流/…`」。
- 反向可以：`交流/` 的筆記可以引用 `技術/` 與 `ai/`。

```yaml
---
tags: [技術, character]
status: 已實作        # 已實作 / 進行中 / 規劃中 / 現況 / 參考
scene: scenes/main.tscn
script: scripts/character/vision.gd
updated: 2026-08-07
---
```

### 寫筆記的規則

- **寫現況，不要寫流水帳。** 事情改了就改內容 ——
  不要在後面追加一節說「前面那段已過時」，那會讓查資料的人讀到錯的東西。
- **不要留「原本是什麼、後來改成什麼」的紀錄。** 更新筆記時直接把舊的那段刪掉、
  換成新的，不要寫成對照（「原本 X → 現在 Y」「這篇推翻了哪幾條」）。
  沿革要查 git log 與 `note/ai/changelog.md`，筆記本身只回答「現在是什麼」。
  一整篇都是過程紀錄的筆記（評估、拍板問答），拍完板就整份刪掉。
- **不要留考古內容。** 已經刪掉的程式碼、已經放棄的方案，除非還在影響現在的決策，
  否則不要在筆記裡描述它們。
- 需要人類拍板的問題寫進 `交流/`，拍板後把結論抄回 `技術/` 或 `ai/`。
- 筆記一律中文，`ai/` 也是。AI 讀中文散文沒有比較吃力，
  改用英文換不到理解力，卻換掉「人類想抽查時看得懂」。
  程式識別字、API 名稱、檔案路徑照舊保持原樣。
- **改動或拿掉一份筆記裡的章節標題時，收工前用 Grep 反查整個 `note/` 有沒有
  其他文件用《文件名》「舊標題」這種形式指過去。**（這是明確例外——只用來抓
  「《文件名》＋舊標題文字」這種 Obsidian CLI 搜尋不到的結構性死連結，不是
  拿 Grep 取代日常的筆記搜尋，平常查閱／管理筆記仍照上面的規則走 `obsidian`
  CLI。）改內容不會留死連結，改標題結構才會——這兩種改動要分開檢查，只核對
  「內容有沒有錯」不會抓到「連結指向的段落已經不存在」，讀的人點過去只會看到
  不相干的地方，卻不知道哪裡出錯。

## 遊戲機制規格：AI 自主性自檢

寫進 note/規格書 前，比照《00 設計原則與架構》原則二自我審查：

> 引擎只給事件，不給情緒。引擎報告發生什麼，AI 決定這算不算一回事。

發現新機制讓引擎預先幫 AI 貼上主觀感受、意圖、或道德/敵對定性的標籤時，
先把問題拋出來讓使用者拍板，不自己直接寫進規格書。

標籤不一定要真的注入給 AI 看才算數——引擎內部用固定公式幫一個動作的性質定性
（例如藉此決定 `relations` 該扣多少），本質上是同一種標籤，一樣要先拋出來，
不要因為它只存在後端運算、或被歸類成「客觀事實」欄位，就以為不受這條規則管。

**不回報也是一種標籤。** 引擎依關係數值等條件判斷「這件事不用讓 AI 知道」，
跟塞一句情緒字串給 AI 看是同一種定性，AI 一樣沒機會表態。純技術性的過濾
（避免同一事件短時間內重複回報）不算，差別在於前者替 AI 決定「這算不算一回事」，
後者只是不重複問——「當下沒有 LLM 可問」不算進這條技術性例外：事件本身仍要留著，
等下次有機會問（fallback、重試、或留給下一輪決策/反思一併處理），不能因為當下
問不到就直接判定「這件事不用讓 AI 知道」而永久丟棄，那樣等於用技術限制包裝了
一次原則二式的預先定性。

**「常發生」不是「不重要」的理由。** 用頻率（怕洗版、怕 LLM 呼叫量爆增）
去省略事件，是成本論證冒充重要性判斷。若理由真的是成本，就明講是成本／
自主性的取捨，讓使用者拍板要不要接受，不要包裝成「這本來就不重要」。

**新增會被當「客觀事實」用的欄位，先核對原則三**：除了餵給 LLM 讀，引擎
自己有沒有拿它算東西？查不到任何公式在用，只有 prompt 組裝端在讀，那就
不符合留存門檻，該交給 AI 自己的記憶系統記、自己判斷。

**這條自我審查不是只在新寫機制時才做一次，讀已有規格書時也要做。** 一段
公式或設計已經寫在文件裡不代表它通過審查——可能是舊規格從沒被人用這條
規則檢查過。查核／改寫既有規格書段落時（不只是新增），只要碰到「引擎依
某個角色的數值/狀態，替另一個角色或這個角色的心智狀態算出一個結論」這
種形狀（例如：從某人的人格值換算出他此刻有多警覺、多信任），都要停下來
比照上面的原則二/三判斷，不要因為只是在「核對用詞正不正確」就跳過，逕自
把舊描述改得更精確、卻沒質疑這個設計本身站不站得住腳。

## 規格書同步 SOP（2026-09-01 全掃 #910~#912 後制定）

規格書腐爛的根因是「同一事實寫在多處＋同步沒有驗收點」——2026-08-31 全掃 26 篇抓出
34 個問題，其中僅 7 個是設計問題，其餘全是同步債（拍板了規格書沒改、程式碼接上了
現況描述過期、同一數值六份文件兩種說法）。防再發靠四條：

1. **事實只在一處定義，硬性執行**：數量、上限、門檻、容量這類斷言，權威定義只出現在
   一個文件（筆數歸《04》§4-6、地點一般容量歸《07》§1-1、死亡禁入例外歸《09》§1-1…），
   其他地方一律寫「見《XX》§N」，不重述數值。同步點隨重述次數線性增長，重述就是債。
2. **拍板的驗收點是規格書同步**：`note/ai/決策紀錄.md` 的「已同步文件」欄必須列到
   文件＋章節（如 `07 §2-3`），不能只寫文件名；規格書現況描述沒改完，該決策不算完成。
3. **PR 自查**（見 `.github/PULL_REQUEST_TEMPLATE.md`）：動到 `IMPLEMENTED_ACTIONS`
   白名單、行為可用性、機制取代時，同一 PR 內修改對應規格書章節——程式碼追上規格時，
   「文件過期」跟程式碼同一個 PR 修，不留給下一雙手。
4. **里程碑收尾全掃**：每個階段（MVP/學期里程碑）收尾時做一次規格書全掃——比照
   2026-08-31 那次：對照《00》四原則＋程式碼現況核對＋跨文件數值 cross-check，
   發現開 issue 收斂後修，不讓債積過一個階段。

## GitHub issue 與 PR

### 開始處理 issue：用 `gh issue develop` 開分支

```
gh issue develop <N> -c -n <branch-name>
```

**不要自己 `git switch -c`。** 手開的分支沒有跟 issue 連動，
issue 頁面上看不到「哪條分支在做這件事」，PR 也不會自動關掉它。
在動第一行程式碼之前就先開分支，不要改到一半才補。

**分支名是 `<type>/<描述>`**，type 用 commit 的那一組（`feat` / `fix` / `refactor` / `docs`），
描述用英文 kebab-case：`fix/chat-input-esc-close`。
`-n` 不要省略 —— 自動生成的名字是 issue 標題整串，長且不帶 type。

分支開錯名字就讓它錯到底，**不要事後改名**。
GitHub 的 rename API 不會把已開的 PR 轉到新分支，它直接把那則 PR 關掉，
只能重開一則新的，等於白白多一個作廢編號。

**標題用英文，內文用中文。** 組員不讀英文，但標題是拿來掃清單的，短而一致比較好認。
程式碼識別字、檔案路徑、指令一律保持原樣，不要翻譯。

### 一則 issue 只寫那一件事

該寫的：**問題是什麼、造成什麼影響、建議怎麼做、相關檔案在哪**。

**不該寫進去的：**

- **對話脈絡。** issue 是給沒參與討論的人讀的。
  「順帶一提，這跟你剛才問的 X 是兩件事」這種句子對他們沒有意義。
- **對別的提案的反駁。** 如果某個做法被否決了，理由屬於技術筆記或 PR 討論，
  不是 issue 內文。issue 描述的是要做什麼，不是曾經考慮過什麼。
- **「順手做一下」的別件事。** 那是另一則 issue。
  夾帶進來之後，關掉這則就等於把夾帶的那件事一起弄丟了。

例外是**範圍界線**：明講「這則不包含 X」是有用的，因為它防止 scope creep。
但只寫界線，不要順著解釋 X 該怎麼做。

### 交叉引用要確認編號

只在真的有依賴關係時才引用別的 issue，而且**寫之前先確認那個編號是對的**。
引錯編號比不引用更糟 —— 讀的人會照著跳過去，然後看到一則不相干的東西。

### PR 審查與指派：Assignee 一定要設；coderabbitai 主審，robot-ru 補位

- **開 PR 時必須把 Assignee 設成自己。** PR 的產權人就是作者；審查未通過時
  PR 要指回作者修，沒有 Assignee 就沒有人接到「該修了」的訊號，PR 會爛在列表裡。
  （CI 會自動補，但補錯了責任還是在作者——自己設才是保證。）
- **開 PR 後 CI 自動 request `coderabbitai` 為主審**：PR 一開就會跑；
  被 rate limit 或 push 後想重審，留言 `@coderabbitai review` 觸發。
  AI 審查抓不到的（實機行為、設計取捨、跨系統語意）記在它的留言裡讓人接手，
  不要預期它全抓。
- **robot-ru 不在自動指派名單**，只在兩種情況接手人工審查：
  1. coderabbitai 沒有出正式審查（額度上限、review 被跳過）；
  2. 它的審查有明顯漏抓（作者或讀者發現具體疑點）。
  接手時由 robot-ru 手動 request 自己或直接提交正式 review。
- **AI 審查只審「被叫的那個版本」**：push 新修正後 AI 不會自動重審。
  作者改完要自己留言 `@coderabbitai review` 觸發重審；留言後看到
  rate limit（額度用完）就表示該 robot-ru 接手了——這是明確訊號，
  不要默默等。
- **審查結果一律用 GitHub 正式 review 狀態記錄，不是留言而已**：
  - `Approved`：通過，可以合併（coderabbitai 或 robot-ru 任一邊 Approved 即算過）。
  - `Changes requested`：未通過，逐項修正。
  作者判斷狀態看 PR 頁面的 review 區塊（Conversation 頁最上方的 review 摘要），
  不要只掃留言。
- 收到 `Changes requested` 修完 push 之後，**回到 review 區塊點
  「Re-request review」**重新排隊。不要開新 PR、不要只留言說改好了——
  正式 review 狀態要靠 re-request 才會重跑。

### 作者沒回應時的代修

審查意見給出後，不需要等，任何人隨時可以代修：

- **代修前先在 PR 留言宣告**要修哪些項目——原作者回來看得見，也避免兩邊同時改。
- **直接 push 到原分支，不開新 PR**；commit 訊息開頭標 `代修:`，讓人一眼分出哪些不是原作者寫的。
- **Assignee 保持原作者不動**，代修身份用留言說明就好。
- **代修者可以 Approve**，但要在 review 留言聲明「本人代修、審查由獨立
  agent session 執行」，讓 PR 頁面看得出這是代修自審。例外：PR 作者本人
  （如 robot-ru 代開的 PR）GitHub 禁止 self-approve，只能交給 coderabbitai。
- 原作者任何時候回來想接手，代修者立即停止。
