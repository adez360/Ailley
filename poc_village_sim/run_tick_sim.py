...
# 2026-08-07：角色卡起始背包裡的狼皮/狼肉/野雞/野雞蛋（見 characters/*.json，都標
# 「明早會爛」，是背景設定的可交易資源，不是穿戴裝備）原本沒被收進這份清單，導致
# 角色帶著這些東西去「賣東西」實際上會失敗（背包裡沒有可賣的東西）——DPO Phase 1
# 幫阿蘭/鐵牛做「賣東西」chosen資料時發現這個落差，這裡補齊，狼皮/狼肉比照獸皮/
# 獸肉同價，野雞/野雞蛋另外訂價（家禽不如野獸值錢，蛋最便宜）。粗布外套/皮背心/
# 破舊襯衫/木笛沒有「會爛」標記，是穿戴裝備/背景道具，不是交易資源，不收進來。
SELL_PRICES = {
    "獸皮": 15, "獸肉": 8, "藥草": 8,
    "狼皮": 15, "狼肉": 8, "野雞": 6, "野雞蛋": 3,
}  # 賣東西的收購價；背包裡沒有可賣的東西就白忙一場
