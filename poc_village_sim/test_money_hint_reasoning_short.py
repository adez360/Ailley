"""推理鷹架實驗（壓縮版，2026-08-09）：跟test_money_hint_reasoning.py同一個假設，
差別只在reasoning欄位加字數上限——目的是讓模型「思考後動作」，不是要拿這段思考
內容去用，字數不用長，只求它有先過一次腦。測看看壓縮後決策品質有沒有掉、
延遲有沒有真的降下來。
"""
import sys
...
_REASONING_INSTRUCTION_SHORT = (
    "\n\n【新增欄位：reasoning，必須寫在最前面，上限40字】\n"
    "在你決定 intent 之前，先用 `reasoning` 欄位簡短寫下：現在最大的問題是什麼、"
    "打算怎麼解決。**不超過40字，只寫結論，不用列出所有考慮過的選項**——這段是"
    "給你自己先想清楚方向用的，不是要寫給別人看的完整分析。\n"
...
des.DURATION_INSTRUCTION = _REASONING_INSTRUCTION_SHORT
