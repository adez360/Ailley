...
**沉默也是一種標籤。** 引擎不只會用「塞一句情緒字串」貼標籤，也會用「決定
要不要讓 AI 知道這件事發生過」貼標籤——如果某段程式碼依某個條件（尤其是
`relations`／`reputation` 這類關係數值）決定要不要把一個事件回報給 AI，
那個「不回報」的判斷本身就是「這件事對你不重要」的定性，AI 連表態的機會
都沒有，跟直接塞一句「這不算什麼」給它看是同一件事。純結構性的過濾（沒有
LLM 可問、正在對話中、同一事件短時間內已經回報過一次避免洗版）不算——差別
在於前者是引擎替 AI 判斷「這算不算一回事」，後者是引擎判斷「有沒有辦法／
有沒有必要重複問」，兩者要分清楚。

**「這種事很常發生」不是「這種事不重要」的理由。** 這是最容易誤判的地方：
頻率是工程／成本論證（問太多次會洗版、會爆 LLM 呼叫量），重要性是原則二
管的範圍，兩者不是同一件事，不能拿前者當後者的理由靜默過濾掉。如果真正的
理由是成本，就把它當成本／自主性的取捨老實拋出來讓使用者拍板（要接受呼叫
量增加、還是接受某類事件永遠不會被回報），不要包裝成「這件事本來就不重要」
悄悄決定掉。

**新機制寫進規格書或程式碼前，順手核對一次原則三。** 一個新欄位如果會被
其他系統當「客觀事實」使用，先問「除了餵給 LLM 讀，引擎自己有沒有拿它算
東西」——查不到任何公式讀取它，只有 prompt 組裝／顯示端在讀，就不符合
「客觀事實」的留存門檻，應該老實承認它其實是主觀感受，交給 AI 自己的記憶
系統記、自己判斷，不要因為它已經是一個獨立的資料庫欄位就預設它一定要留著。
