...
## MigrationV7Test
##
## 驗證 issue #561／DatabaseSchema migration 7：既有資料庫（缺 NOT NULL
...
## home_location_id），npc 則有近 20 張表外鍵指向它，其中 grave／memories
## 還各自帶一張自己的子表（grave_epitaphs／memory_related_npcs）——改名
## location／npc 時，SQLite 會把全部這些表的外鍵定義自動改指向暫存表名，
## 這裡種好整條依附鏈的資料，驗證重建後全部表的資料都保留、外鍵都改指向
## 新表而不是停留在暫存表名。
##
## 使用方式：
## 1. 將本檔放到：
##      res://scripts/database/MigrationV7Test.gd
## 2. 建立一個暫時測試場景：
##      Node
##        └── MigrationV7Test
## 3. 將本腳本掛在 MigrationV7Test。
## 4. 執行測試場景。
##
## 注意：
...
##   既有資料庫的欄位定義，這是 migration 7 保證會修好的情形。其餘依附表
##   本身主鍵沒有 NOT NULL 缺口（migration 3／6 已修過，或本來就是
##   INTEGER PRIMARY KEY AUTOINCREMENT／已宣告 NOT NULL 的複合主鍵），
##   直接呼叫對應 *Schema.gd 的 create(db) 建立即可，不需要另外手刻舊版
##   SQL，也不會跟真實 schema 定義漂移。
## - user_version 設成 6（模擬「已經套用過 migration 2-6，只差 7」），
##   跟 MigrationV6Test 設成 5 是同一套模擬手法。
## =====================================================


const TEST_DB_PATH := "user://__migration_v7_test.db"

const LOCATION_A := "__migration_v7_test_loc_a"
const NPC_A := "__migration_v7_test_npc_a"
const NPC_B := "__migration_v7_test_npc_b"
const WORLD_A := "__migration_v7_test_world_a"
const ITEM_A := "__migration_v7_test_item_a"
const MEMORY_A := "__migration_v7_test_memory_a"
...
## NOT NULL 拿掉——npc_state 本身在 migration 6 已經修過 NOT NULL，這裡
## 刻意重現「修過之前」的舊形狀，只用來驗證 migration 7 對它的
## repair_null_pk=false（預設）defensive 判斷本身是正確的，不代表這個
## table 在真正的 migration 鏈（3→7 依序套用）裡還可能出現這個缺口。
...
func _seed_legacy_schema() -> bool:
	...
	if not db.query(_legacy_location_sql()):
		push_error("[MigrationV7Test] seed location 建表失敗: " + db.error_message)
		return false

	# --- npc（舊版：npc_id 缺 NOT NULL）---
	if not db.query(_legacy_npc_sql()):
		push_error("[MigrationV7Test] seed npc 建表失敗: " + db.error_message)
		return false
	...
	if not WorldSchema.create(db) or not ItemSchema.create(db):
		push_error("[MigrationV7Test] seed world/item 失敗: " + db.error_message)
		return false

	# 全部依附表本身主鍵沒有 NOT NULL 缺口（已在 migration 3／6 修過，或本來
	...
	var dependent_schemas := [
		...
		ItemTransactionSchema, NPCAppearanceSchema, NPCConditionSchema,
	...
	for schema in dependent_schemas:
		if not schema.create(db):
			push_error("[MigrationV7Test] seed 依附表建表失敗: " + db.error_message)
			return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed location 資料失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed npc 資料失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO world (world_id) VALUES (?);", [WORLD_A]
	):
		push_error("[MigrationV7Test] seed world 資料失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO item (item_id, name) VALUES (?, ?);", [ITEM_A, "測試道具"]
	):
		push_error("[MigrationV7Test] seed item 資料失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed grave 失敗: " + db.error_message)
		return false

	if not db.query("SELECT last_insert_rowid() AS id;"):
		push_error("[MigrationV7Test] 讀 grave_id 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed grave_epitaphs 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed memories 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed memory_related_npcs 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed money_transaction 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed item_transaction 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_appearance (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_appearance 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_condition (npc_id, type) VALUES (?, ?);", [NPC_A, "sleepy"]
	):
		push_error("[MigrationV7Test] seed npc_condition 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_daily_plan (npc_id, game_day) VALUES (?, ?);", [NPC_A, 1]
	):
		push_error("[MigrationV7Test] seed npc_daily_plan 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_emotion (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_emotion 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_goal (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_goal 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed npc_home_storage 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed npc_inventory 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_last_action (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_last_action 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_occupation (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_occupation 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_personality (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_personality 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed npc_relations 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed npc_schedule 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_state (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_state 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_taboo (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_taboo 失敗: " + db.error_message)
		return false

	if not db.query_with_bindings(
		"INSERT INTO npc_wallet (npc_id) VALUES (?);", [NPC_A]
	):
		push_error("[MigrationV7Test] seed npc_wallet 失敗: " + db.error_message)
		return false
	...
	if not db.query_with_bindings(
		...
		push_error("[MigrationV7Test] seed world_character_state 失敗: " + db.error_message)
		return false

	if not db.query("PRAGMA user_version = 6;"):
		push_error("[MigrationV7Test] 設定 user_version 失敗: " + db.error_message)
		return false
...
## （issue #566／P-59，跟 migration 6 的 world 同一套判斷）。種各一筆主鍵
...
## repair_null_pk=false，不能亂補（issue #566／P-59，跟 migration 6 對
...
func _run_null_pk_reject_test() -> void:
	...
	_check(
		"中止後 user_version 保持在 6（ROLLBACK，不是半套）",
		_get_user_version() == 6,
		"got %d, expected 6" % _get_user_version()
	)
...
func _can_insert_orphan_npc_wallet() -> bool:
	return db.query_with_bindings(
		...
		["__migration_v7_test_nonexistent_npc"]
	)
...
func _null_pk_row_repaired(
	...
	var repaired_id: String = db.query_result[0].get(pk_column, "")
...
func _finish() -> void:
	...
	print("")
	print("=====================================================")
	print("[MigrationV7Test] RESULT")
	print("=====================================================")
	print("PASS: ", passed)
	print("FAIL: ", failed)

	if failed == 0:
		print("[MigrationV7Test] ALL TESTS PASSED")
	else:
		push_error("[MigrationV7Test] %d test(s) failed." % failed)
