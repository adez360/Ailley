...
# {"action": "work"} 對應到一個 Workstation 實例，而它需要節點參照，
# player.gd 的候選偵測也只看 32px 範圍內、面向著的物件。列進來的話就變成
# 「白名單宣稱做得到、實際靜默不做」，正是上面那段註解要避免的混淆。
# 等執行層接得到再加（talk 的動作執行留給 #90，其餘留給各自的 issue）
#
# buy 已於 #340 接上執行層（agent.gd::_pursue_buy_task()），列進下面的清單
#
