...
func _unhandled_input(event: InputEvent) -> void:
	...
	elif not shop_place.is_empty() and candidates["to_shop"] <= candidates["to_other"] and vending_menu != null:
...
## 找出目前附近的三種互動候選（工作站／商店／可搭話的人）跟各自的距離。
...
## `_is_facing()` 把沒面向的候選直接排除，玩家沒面向任何東西時三個候選都是
...
func _process(_delta: float) -> void:
	...
	elif not String(candidates["shop_place"]).is_empty() and candidates["to_shop"] <= candidates["to_other"] \
