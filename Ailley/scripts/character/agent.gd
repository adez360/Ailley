...
func _pursue_work_too_far_retry() -> void:
	...
	if reason == Character.WORK_OCCUPIED:
		# 跟上面「走去地點站位」那段的 WORK_OCCUPIED 分支同一套規則（issue
		# #868）：這裡是 work_at() 的第二個呼叫點，同樣的「失敗但什麼都不
		# 記錄」風險一樣存在，不能因為是重試路徑就漏接
...
# 仍失敗），抽出來避免重複。issue #868：一併記進跨任務失敗計數、llm 來源
# 的任務要離開池子（不留著等退避解除，跟 #937 的既有處理一致），不然這個
# 共用收尾又會變回「失敗但什麼都不記錄」
...
func _pursue_buy_task() -> void:
	...
	# 另外找一個機台節點
	if not Shop.has_shop(place) or anchors == null or not anchors.has_for(self, place):
