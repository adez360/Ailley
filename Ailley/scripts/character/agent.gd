...
##
## wash 不列在這裡：`Stats.SPEC` 已有 `hygiene`，但 wash 動作本身還沒接上
## 任何回復邏輯（#229），現在 hygiene 會照 drift 往 0 掉、卻沒有動作能補回來，
## 待補。idle 也不列——發呆本來就不回復任何東西，它的用途是「合法地什麼都不做」，
## 讓 AI 逾時或沒事可做時有一個不必假裝在忙的選項
