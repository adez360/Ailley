...
## 這是工程上的安全閥——雙方都不主動收尾時避免真的無限跑下去，同時也是無觀眾
## 世界的 LLM 呼叫成本閘門（#178）：本機小模型多輪對話品質實測 6 輪起始
## 退化、10 輪偶爾明顯退化，超過這個範圍的對話多半已經不值得再付費續下去。
## 10 是三份獨立證據（實測、poc_village_sim 導演模式 B、poc_village_sim 逐
## tick 對話追蹤）的保守交集值，不是精算出來的——記憶注入上線後每輪 payload
## 都會變大，退化點可能提前，屆時應針對現在的逐輪架構重新實測
